...
function homeassistant-dev-show-short-info {
  echo "Home Assistant beta install script for Hassbian."
}

function homeassistant-dev-show-long-info {
  echo "Installs Home Assistant beta onto this system."
}
...
echo "Installing latest beta version of Home Assistant"
...
if [ ! -z "${validation}" ]; then
  ...
  echo "Your Home Assistant beta installation is running at $ip_address:8123 or if preferred http://hassbian.local:8123"
  echo
else
  echo
